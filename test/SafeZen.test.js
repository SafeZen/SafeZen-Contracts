...
describe('Activating a flow', async function () {
  it('User starts a flow - reflected correctly in balance', async () => {
    ...
    assert.equal(
      SafeZenFlowRate,
      '70000000',
      'SafeZen is not receiving 100% of the flowRate'
    );
    assert.equal(
      ownerFlowRate,
      '-70000000',
      'OwnerFlowRate is not reflected correctly'
    );
    assert.equal(
      SafeZenFinalBalance - SafeZenInitialBalance,
      SafeZenFlowRate * TEST_TRAVEL_TIME,
      'Wrong balance from flowrate'
    );
  });
  ...
  it('Policy should be started', async () => {
    ...
    assert.equal(
      isPolicyActive,
      true,
      'Policy should be activated with the flow'
    );
  });
  ...
  it('Policy should not be active if no flow from owner', async () => {
    ...
    assert.equal(
      isPolicyActive,
      false,
      'Policy should not be activated with no flow'
    );
  });
